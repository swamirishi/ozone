...
class RDBDifferComputer extends FileLinkDeltaFileComputer {
  ...
  public Optional<Map<Path, Pair<Path, SstFileInfo>>> computeDeltaFiles(SnapshotInfo fromSnapshot,
    ...
    if (differ != null) {
      try (OmSnapshotLocalDataManager.ReadableOmSnapshotLocalDataProvider snapProvider =
        ...
        synchronized (differ) {
          Optional<Map<Path, SstFileInfo>> paths = differ.getSSTDiffListWithFullPath(toDSI, fromDSI, versionMap, tablePrefixInfo,
              tablesToLookup);
          if (paths.isPresent()) {
  ...
  private static DifferSnapshotInfo getDSIFromSI(OMMetadataManager activeOmMetadataManager,
    ...
    NavigableMap<Integer, List<SstFileInfo>> versionSstFiles = snapshotLocalData.getVersionSstFileInfos().entrySet().stream()
        .collect(toMap(Map.Entry::getKey,
            entry -> entry.getValue().getSstFiles(),
