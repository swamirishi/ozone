 ...
 * GET and POST requests to external resource.
 */
public class SolrHttpClient {
  ...
  public static SolrHttpClient getInstance() {
    if (instance == null) {
      synchronized (SolrHttpClient.class) {
        if (instance == null) {
          instance = new SolrHttpClient();
        }
      }
    }
    return instance;
  ...
  public String sendPost(String host, int port,
    ...
    try {
      ...
      LOG.info(
          "Status Code: {}. Time taken by URI to return: {} " +
  ...
  private String getHostBaseURI(String host, int port, String uri) {
