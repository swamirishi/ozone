...
public class TestPipelineClose {
  ...
  void testPipelineCloseTriggersSkippedWhenAlreadyInProgress() throws Exception {
    ...
    GenericTestUtils.setLogLevel(XceiverServerRatis.LOG, Level.DEBUG);
    GenericTestUtils.LogCapturer xceiverLogCapturer = GenericTestUtils.LogCapturer.captureLogs(XceiverServerRatis.LOG);
    ...
    try {
      ...
      for (int i = 1; i <= numOfDuplicateTriggers; i++) {
        xceiverRatis.triggerPipelineClose(groupId, detail + i, ClosePipelineInfo.Reason.PIPELINE_FAILED, true);
      }
      ...
      Assert.assertEquals(numOfDuplicateTriggers, skippedCount);
    } finally {
