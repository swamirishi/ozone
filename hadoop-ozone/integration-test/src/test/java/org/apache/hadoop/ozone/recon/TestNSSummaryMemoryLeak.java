 ...
 * @see ReconNamespaceSummaryManager#batchDeleteNSSummaries
 */
  ...
  private void verifyNSSummaryEntriesExist(ReconOMMetadataManager omMetadataManager,
    ...
    GenericTestUtils.waitFor(() -> {
      try {
        ...
        try (TableIterator<?, ? extends Table.KeyValue<?, OmDirectoryInfo>> iterator = dirTable.iterator()) {
          while (iterator.hasNext()) {
            Table.KeyValue<?, OmDirectoryInfo> kv = iterator.next();
            dirCount++;
