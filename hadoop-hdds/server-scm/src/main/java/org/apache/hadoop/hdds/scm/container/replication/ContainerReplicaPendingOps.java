...
public class ContainerReplicaPendingOps {
  ...
  private final ConcurrentHashMap<UUID, SizeAndTime> containerSizeScheduled = new ConcurrentHashMap<>();
  ...
  private void releaseScheduledContainerSize(ContainerReplicaOp op) {
    containerSizeScheduled.computeIfPresent(op.getTarget().getUuid(), (k, v) -> {
  ...
  private void addReplica(ContainerReplicaOp.PendingOpType opType,
    ...
    try {
      ...
      if (opType == ADD) {
        containerSizeScheduled.compute(target.getUuid(), (k, v) -> {
          if (v == null) {
      ...
      if (ops != null) {
        ...
        while (iterator.hasNext()) {
          ...
          if (op.getOpType() == opType
            ...
            iterator.remove();
            if (opType == ADD) {
              containerSizeScheduled.computeIfPresent(target.getUuid(), (k, v) -> {
                long newSize = v.getSize() - op.getContainerSize();
                if (newSize <= 0) {
                  return null;
                }
  ...
  public ConcurrentHashMap<UUID, SizeAndTime> getContainerSizeScheduled() {
